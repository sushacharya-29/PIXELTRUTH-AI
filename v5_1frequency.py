...
class DCTLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # cols
        x = torch.einsum('kj,bciw->bcik', self.basis, x)
        return x
...
class LearnedFrequencyDecomposer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Project real and imaginary spectra back to spatial domain separately.
        # FIXED: Use proper complex tensors for irfft2.
        # real_n and imag_n are both real-valued 2D spectra (H x W//2+1).
        # We reconstruct a valid complex spectrum for each and invert it.
        # real path: treat real_n as the real part, zero imaginary.
        # imag path: treat imag_n as the real part, zero imaginary.
        # This gives meaningful spatial maps without garbage from malformed spectra.
        real_spatial = torch.fft.irfft2(
            torch.complex(real_n, torch.zeros_like(real_n)),
            s=(self.image_size, self.image_size), norm='ortho'
        )
        imag_spatial = torch.fft.irfft2(
            torch.complex(imag_n, torch.zeros_like(imag_n)),
            s=(self.image_size, self.image_size), norm='ortho'
        )

        # Clamp tightly before sending into convs — AMP fp16 overflows above ~65504.
        # 5.0 gives >4 sigma headroom for typical frequency maps.
        real_spatial = real_spatial.clamp(-5.0, 5.0).to(x.dtype)
        imag_spatial = imag_spatial.clamp(-5.0, 5.0).to(x.dtype)
